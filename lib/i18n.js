...
class I18n {
    constructor(locale,lang_path) {
        ...
        this._lang_path = lang_path || "translations";
        this._strings = null;

        console.log("Init I18n",locale,this._lang_path)
    }
    ...
    init() {
        ...
        try {
            this._strings = this._readStringsFile(__dirname, '..', '.', this._lang_path, `${this.locale()}.json`);
        } catch (err) {
            this._strings = null;
            throw err;
        }
    ...
    _formatMessage(string, bindings) {
        ...
        // Escape HTML tags so intl-messageformat doesn't interpret them as variables
        // We'll use a placeholder that's unlikely to conflict
        const htmlTagMap = {};
        let escapedString = string;
        let tagIndex = 0;
        
        // Replace HTML tags with placeholders
        escapedString = escapedString.replace(/<(\/?)([a-zA-Z][a-zA-Z0-9]*)([^>]*)>/g, (match, closing, tagName, attributes) => {
            const placeholder = `__HTML_TAG_${tagIndex}__`;
            htmlTagMap[placeholder] = match;
            tagIndex++;
            return placeholder;
        });
        
        let msg = new IntlMessageFormat(escapedString, currentLocale);

        try {
            msg = msg.format(bindings);
            
            // Restore HTML tags
            for (const placeholder in htmlTagMap) {
                msg = msg.replace(placeholder, htmlTagMap[placeholder]);
            }
        } catch (err) {
            console.error(new Date(),"i18n _formatMessage",err.message);

            // fallback
            msg = new IntlMessageFormat(this._fallbackError(), currentLocale);
            msg = msg.format();
        }
