...
class I18n {
    ...
    _formatMessage(string, bindings) {
        ...
        // Debug logging for product page placeholders
        if (string && string.indexOf('{cant_contratos}') !== -1) {
            console.log('=== i18n._formatMessage DEBUG ===');
        ...
        try {
            ...
            if (string && string.indexOf('{cant_contratos}') !== -1) {
                console.log('Formatted output:', msg);
                console.log('=== END DEBUG ===');
            }
        } catch (err) {
            console.error(new Date(),"i18n _formatMessage ERROR:", err.message);
            console.error('String:', string);
            console.error('Bindings:', JSON.stringify(bindings, null, 2));
