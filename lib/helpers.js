...
const helpers = {
  _: function (text, options) {
    ...
    try {
      if (!options.data.root.settings) {
        options.data.root.settings = {};
      }

      if (!options.data.root.settings["i18n-" + lang]) {
        const langPath =
          options.data.root.general &&
          options.data.root.general.lang_path &&
          options.data.root.general.lang_path[0] &&
          options.data.root.general.lang_path[0].staging
            ? options.data.root.general.lang_path[0].staging
            : "translations";
        options.data.root.settings["i18n-" + lang] = new I18n(lang, langPath);
      }
      ...
      if (options.hash) {
        ...
        for (prop in options.hash) {
          if (Object.prototype.hasOwnProperty.call(options.hash, prop)) {
            let value = options.hash[prop];

            // Convert to string if needed, handle undefined/null
            if (value === undefined || value === null) {
              bindings[prop] = "";
            } else if (value && typeof value === "object") {
              // Handle Handlebars SafeString objects and other objects
              // SafeString has a toString method that returns the string value
              if (value.toString && typeof value.toString === "function") {
                const stringValue = value.toString();
                // If toString returns an object, try to get the string property
                if (typeof stringValue === "string") {
                  bindings[prop] = stringValue;
                } else if (value.string && typeof value.string === "string") {
                  bindings[prop] = value.string;
                } else {
                  bindings[prop] = String(value);
                }
              } else if (value.string && typeof value.string === "string") {
                // Direct access to SafeString.string property
                bindings[prop] = value.string;
              } else {
                bindings[prop] = String(value);
              }
  ...
  lang: function (options) {
    ...
    const default_lang = options.data.root.general.default_lang
      ? options.data.root.general.default_lang[0][helpers.env()]
      : "es";
  ...
  url: function (options) {
    ...
    function compile(components) {
      ...
      let view_url = "";
      let view = findViewByUrl(components.view);
      if (view) {
        ...
        if (!view_url) {
          console.error(
            "url compile",
            "Called view with undefined lang",
            components.lang,
          );
      ...
      for (component_index in Object.keys(components)) {
        ...
        if (component.slice(0, 1) == "_") {
          ...
          if (component == "_anchor") {
            ...
            let replacementComponent = components[component]
              ? encodeURIComponent(components[component])
              : "";
    ...
    if (!options.data) {
      console.error(
        "url helper called without options.data (check for parameters without name)",
        options,
      );
      return options;
    }
  ...
  get_source_description(source, options) {
    ...
    if (options.data.root.general["source_description_" + source]) {
      return options.data.root.general["source_description_" + source][
        current_lang
      ];
    } else {
      return source;
  ...
  conf: function (option, options) {
    if (options.data.root.general && options.data.root.general[option]) {
      const value = options.data.root.general[option][0][helpers.env()];
      // Return empty string for empty/undefined values to avoid "undefined" in string concatenation
      return value || "";
    } else {
  ...
  autocomplete_url: function () {
    return process.env.AUTOCOMPLETE_URL;
  },
  adsense_network_code: function () {
    return process.env.ADSENSE_NETWORK_CODE;
  },
  taboola_enabled: function () {
    return process.env.TABOOLA_ENABLED === "true";
  },
  taboola_publisher_id: function () {
    return process.env.TABOOLA_PUBLISHER_ID || "";
  },
  taboola_loader_url: function () {
    return process.env.TABOOLA_LOADER_URL || "";
  },
  adplus_enabled: function () {
    return process.env.ADPLUS_ENABLED === "true";
  },
  adplus_website: function () {
    return process.env.ADPLUS_WEBSITE || "quienesquien.wiki";
  },
  ...
  format_amount: function (
    ...
    if (value) {
      ...
      if (value > 10) {
        formatted = nt.value.toLocaleString("es-MX", {
          style: "decimal",
      ...
      if (originalValue && originalCurrency) {
        formatted += " (" + originalValue.toLocaleString("es-MX");
        if (options) {
          formatted += " " + originalCurrency;
        }
        if (exchangeRate) {
          formatted +=
            ' <i class="fas fa-info-circle text-gray"  title="' +
            helpers._(
              "Contrato en moneda extranjera. Tasa de conversión del día de firma del contrato",
              options,
            ) +
            ":  1" +
            originalCurrency +
            " = " +
            exchangeRate.toLocaleString("es-MX", {
              style: "decimal",
              maximumFractionDigits: 1,
            }) +
            currency +
  ...
  j: function (obj) {
    return util
      .inspect(obj, { depth: 5, maxArrayLength: 10000 })
      .replace(RegExp("&#x27;", "g"), '"')
      .replace(RegExp("&#x27;", "g"), '"');
  },
  ...
  contract_category_min: function (category) {
    return constants.contract_categories_min_max[
      "min-contract_score-" + category
    ];
  },
  contract_category_max: function (category) {
    return constants.contract_categories_min_max[
      "max-contract_score-" + category
    ];
  },
  party_category_min: function (category) {
    ...
    switch (category) {
      case "conf":
        selector = "node_rules-conf";
        break;
      case "comp":
        selector = "node_categories-comp";
        break;
      case "traz":
        selector = "node_categories-traz";
        break;
      case "temp":
        selector = "contract_categories-temp";
        break;
      case "trans":
        selector = "contract_categories-trans";
        break;
    }
  ...
  contract_flag_recommendations: function (flags, category) {
    let recommendations = [];
    for (f in flags.rules_score[category]) {
      if (constants.flag_details[f]) {
        let recommendation = _.clone(constants.flag_details[f]);
        recommendation.score = flags.rules_score[category][f];
        recommendations.push(recommendation);
      } else {
        console.error("contract_flag_recommendations", category, f);
      }
    }
  ...
  get_party_categories: function (flags) {
    ...
    if (flags.contract_categories) {
      return {
        ...
        conf: flags.node_categories.conf,
      };
    }
    // else if (flags.contract_score) {
    //   return {
    //     "trans": flags.contract_score.trans,
    //     "temp": flags.contract_score.temp,
    //     "comp": flags.node_score.comp,
    //     "traz": flags.node_score.traz,
    //     "conf": flags.node_rules["conf-index"],
    //   }
    // }
    else {
  ...
  flag_recommendations: function (org_flags, category) {
    ...
    function get_min(flag_id) {
      if (
        constants.party_categories_min_max["min-node_rules-" + flag_id] !==
        undefined
      ) {
        return constants.party_categories_min_max["min-node_rules-" + flag_id];
      } else {
        return constants.party_categories_min_max[
          "min-contract_rules-" + flag_id
        ];
      }
    }

    // const allFlags = _.merge(org_flags[0].node_rules,org_flags[0].contract_rules);
    const allFlags = _.merge(org_flags.node_rules, org_flags.contract_rules);
    // console.log("allFlags",allFlags);
    _.forEach(allFlags, (i, flag_id, flags) => {
      // console.log(flag_id, category, flag_id.indexOf(category))
      //Seleccionar las que son de la categoría
      if (flag_id.indexOf(category) == 0) {
    ...
    if (category == "conf") {
      const recommendation = _.clone(constants.flag_details["conf"]);
      recommendation.score = allFlags["conf-index"];
      recommendation.minimum = get_min("conf");
      recommendations.push(recommendation);
    }
  ...
  get_object_element: function (object, element) {
    console.log(
      "get_object_element - replace with lookup - called for",
      object,
      element,
    );
    if (object) {
  ...
  format_currency: function (value, options) {
    if (["MX", "MXN"].includes(value)) {
      ...
      return (
        '<i class="fas fa-info-circle text-gray"  title="' +
        helpers._(
          "Este importe no especifica moneda, los valores podrían estar en varias monedas.",
          options,
        ) +
        '"></i>'
      );
  ...
  get_record_flags: function (contract_flags) {
    ...
    for (contract in contract_flags) {
      record_flags.total_score +=
        contract_flags[contract].contract_score.total_score;
  ...
  get_classification_type: function (classification) {
    ...
    switch (classification) {
      ...
      case "Ropa y Telas":
        return "products";
      case "Material médico":
        return "products";
      case "Artículos de consumo":
        return "products";
      case "Medicinas y vacunas":
        return "products";
      case "Mobiliario":
        return "products";
      ...
      default:
        console.error(
          "get_classification_type: Unknown classification:",
          classification,
        );
        return false;
    }
  ...
  cobranding: function (sources, unique_field) {
    const brands = [];
    const sources_list = [];
    if (typeof unique_field == "object") {
      unique_field = "data_name";
    }
  ...
  get_classification_name: function (
    type,
    classification,
    subclassification,
    govLevel,
    ...
    if (
      constants.classification_names[type] &&
      constants.classification_names[type][classification]
    ) {
      if (
        !constants.classification_names[type][classification][
          subclassification
        ] &&
        constants.classification_names[type][classification]["undefined"]
      ) {
    ...
    if (
      constants.classification_names[type] &&
      constants.classification_names[type][classification] &&
      constants.classification_names[type][classification][subclassification] &&
      constants.classification_names[type][classification][subclassification][
        govLevel
      ]
    ) {
      ...
      console.log(
        "get_classification_name unknown:",
        type,
        classification,
        subclassification,
        govLevel,
  ...
  get_classification_icon: function (
    classification,
    subclassification,
    govLevel,
  ) {
    ...
    if (
      classification &&
      product_classifications.indexOf(classification[0]) > -1
    ) {
      subclassification = "undefined";
    }

    if (
      constants.classification_icons[getValue(classification)] &&
      constants.classification_icons[getValue(classification)][
        getValue(subclassification)
      ] &&
      constants.classification_icons[getValue(classification)][
        getValue(subclassification)
      ][getValue(govLevel)]
      ...
      icon =
        constants.classification_icons[classification][subclassification][
          govLevel
        ];
    } else {
      console.log(
        "get_classification_icon unknown:",
        classification,
        subclassification,
        govLevel,
      );
  ...
  get_role_name: function (type, directionality, options) {
    ...
    if (
      constants.role_names[type] &&
      constants.role_names[type][directionality]
    ) {
      // console.log("get_role_name found",type,role_names[type][directionality].label);
      return helpers._(
        constants.role_names[type][directionality].label,
        options,
      );
    } else {
  ...
  get_type_url: function (type, classification, collection, recursion) {
    // Contract records (passed explicitly with collection="contracts") always link
    // to the contract page. Their top-level "type" field is a document/source tag
    // (e.g. "qqw"), not an entity classification -- unrelated to person/company/institution.
    ...
    if (!type) {
      ...
      return helpers.get_type_url(classification, "", collection, recursion);
    }
    ...
    switch (type) {
      case "organizations":
        return helpers.get_type_url(classification);
      case "institutions":
        return "instituciones-publicas/:id";
      case "institution":
        return "instituciones-publicas/:id";
      case "municipality":
        return "regiones/:id";
    ...
    if (records) {
      ...
      if (!party) {
        party = _.find(records.parties, (party) => {
          ...
          if (party.memberOf) {
            return party.memberOf[0].id == party_id;
          }
  ...
  translate_area: function (value, lang) {
    ...
    if (value == "Mexico") {
      return "México";
    } else if (value == "Mexico City" && (lang == "es" || !lang)) {
      return "Ciudad de México";
    } else if (countries.isValid(value)) {
      return countries.getName(value, lang);
    } else {
      // console.log("translate_area unknown:",value);
      return value;
    }
  },
  get_country_id: function (value) {
    let co = "";
    if ((co = countries.getAlpha2Code(value, "es"))) {
      return co.toLowerCase();
    } else {
      if (value) {
        console.log("get_country_id unknown:", value);
        return value;
      }
    }
  },
  get_govlevel_name: function (govlevel, options) {
    return (
      helpers._(constants.govlevel_names[govlevel], options) ||
      govlevel + helpers._(" (desconocido)", options)
    );
  },
  get_bool: function (bool, options) {
    if (bool) {
      return helpers._("Si", options);
    } else {
      return helpers._("No", options);
    }
  },
  ...
  generate_consejeras_por_empresa: function (result) {
    ...
    for (b in result.summaries["2020"].consejeras_por_empresa) {
      let newBucket = {};
      newBucket.consejeras_count =
        result.summaries["2020"].consejeras_por_empresa[b].consejeras_count;
      newBucket.datos2020 =
        result.summaries["2020"].consejeras_por_empresa[b].companies_count;
      newBucket.datos2019 = 0;
      buckets[newBucket.consejeras_count] = newBucket;
    }
    if (!ec) {
      for (b in result.summaries["2019"].consejeras_por_empresa) {
        let consejeras_count =
          result.summaries["2019"].consejeras_por_empresa[b].consejeras_count;
        if (buckets[consejeras_count]) {
          buckets[consejeras_count].datos2019 =
            result.summaries["2019"].consejeras_por_empresa[b].companies_count;
        } else {
          let newBucket = {};
          newBucket.consejeras_count =
            result.summaries["2019"].consejeras_por_empresa[b].consejeras_count;
  ...
  get_publisher_logo: function (publisher_name) {
    if (constants.publisher_logos[publisher_name]) {
      return (
        "/mujeresenlabolsa/images/" + constants.publisher_logos[publisher_name]
      );
    } else {
  ...
  fix_years: function (flags) {
    // console.log(flags);
    if (flags) {
      let years = _.clone(flags.years);
      years = _.reverse(_.sortBy(years, ["year"]));
      // delete flags.years;
      years.unshift(flags);
      flags.year = "Todos";
      return years;
    } else {
      console.error("no flags");
    }
  },
};
