...
const helpers = {
  _: function(text, options) {
    ...
    try {
      ...
      if (!options.data.root.settings["i18n-"+lang]) {
        const langPath = options.data.root.general && 
  ...
  adsense_network_code: function() { 
    // #region agent log
    const code = process.env.ADSENSE_NETWORK_CODE;
    try{fs.appendFileSync('/home/faras/www/qqw-poppins/.cursor/debug.log',JSON.stringify({location:'helpers.js:262',message:'adsense_network_code helper called',data:{hasCode:!!code,codeLength:code?code.length:0,codeValue:code?code.substring(0,20)+'...':null},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})+'\n');}catch(e){}
    // #endregion
    return code; 
  },
  ...
  format_amount: function(value, currency, originalValue, originalCurrency, exchangeRate, options) {
    let formatted = "";
    if (value) {
      ...
      if (originalValue && originalCurrency) {
        ...
        if (exchangeRate) {
          formatted+=' <i class="fas fa-info-circle text-gray"  title="'+helpers._("Contrato en moneda extranjera. Tasa de conversión del día de firma del contrato",options)+":  1"+originalCurrency+" = "+exchangeRate.toLocaleString('es-MX', {style: "decimal", maximumFractionDigits: 1})+currency+'"></i>';
        }
  ...
  get_classification_type: function(classification) {
    ...
    switch (classification) {
      ...
      case "Otros productos": return "Otros productos";
  ...
  get_classification_icon: function(classification,subclassification,govLevel) {
    ...
    if (classification && product_classifications.indexOf(classification[0]) > -1) {
      subclassification = "undefined";
    }
  ...
  get_type_url: function(type,classification,recursion) {
    ...
    switch(type) {
      ...
      case "contract_flags": return "contratos/:id";
