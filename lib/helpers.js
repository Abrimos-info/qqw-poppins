...
const helpers = {
  ...
  url: function (options) {
    ...
    function compile(components) {
      ...
      // Get base path from context for subfolder hosting support
      // This should be set by the middleware in res.locals
      const basePath = options.data.root.basePath || "";
      
      // Ensure we don't create double slashes
      if (basePath && basePath.endsWith("/") && url.startsWith("/")) {
        url = basePath.slice(0, -1) + url;
      } else if (basePath && !url.startsWith(basePath)) {
        url = basePath + url;
      }

      let url = basePath + "/" + components.lang + "/" + view_url;
  ...
  basePath: function (options) {
    const basePath = options.data.root.basePath || "";
    // If argument provided, append it to base path
    if (options.hash && options.hash.path) {
