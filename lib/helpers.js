  ...
  _: function(text, options) {
    ...
    try {
      if (!options.data.root.settings["i18n-"+lang]) {
        options.data.root.settings["i18n-"+lang] = new I18n(lang, options.data.root.general.lang_path[0].staging);
      }
      ...
      if (options.hash) {
        ...
        for (prop in options.hash) {
          if (Object.prototype.hasOwnProperty.call(options.hash, prop)) {
              const value = options.hash[prop];
              
              // Debug logging for product page
              if (text && text.indexOf('{cant_contratos}') !== -1) {
                console.log('Helper _: Processing binding', prop, '=', value, 'type:', typeof value);
              }
      ...
      // Debug logging for product page
      if (text && text.indexOf('{cant_contratos}') !== -1) {
        console.log('Helper _: Final bindings:', JSON.stringify(bindings, null, 2));
        console.log('Helper _: Text:', text);
      }
