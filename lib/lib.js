    ...
    try {
      const req = https.get(CSVurl, response => {
        ...
        response.on("end", function() {
          try {
            parsedCSV = csv.parse(responseData);
          } catch(csvError) {
        ...
        response.on("error", function(error) {
          console.error(`appLocalsFromCSV response error for ${namespace}:`, error);
          app.locals[namespace] = {};
          resolve(); // Resolve with empty data instead of rejecting
        })
      });
      ...
      req.on("error", function(error) {
        console.error(`appLocalsFromCSV request error for ${namespace}:`, error);
        app.locals[namespace] = {};
        resolve(); // Resolve with empty data instead of rejecting
      });
      ...
      req.setTimeout(10000, function() {
        console.error(`appLocalsFromCSV timeout for ${namespace} after 10s`);
        req.destroy();
        app.locals[namespace] = {};
        resolve(); // Resolve with empty data instead of rejecting
      });
