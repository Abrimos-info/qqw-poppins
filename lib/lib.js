...
function redirectToSite() {
  return catchError(async function(req, res, next) {
    ...
    const sitename = hostParts[0].toLowerCase();
    
    if (constants.qqw_sites[sitename]) {
      return redirectToLanguage(constants.qqw_sites[sitename]["es"],constants.qqw_sites[sitename]["en"])(req, res, next);
    }
    else {
...
function redirectToLanguage(es,en) {
  return catchError(async function(req, res, next) {
    ...
    const absoluteURL = `${protocol}://${host}${newURL}`;
