...
function loadSettings(app,callback) {
  ...
  Promise.all([
    ...
    sources_cache(app,"sources").then(()=>{
      ...
      console.error("Error loading sources (continuing without sources)",e.message || e)
      app.locals["sources"] = { data: [] };
      callback(app.locals);
    })
  }).catch(e => {
...
async function package_version(app,namespace) {
  ...
  app.locals[namespace] = {
    ...
    commit: commit,
    commit_short: commit.substr(0,7)
  }
}
...
function redirectToSite() {
  return catchError(async function(req, res, next) {
    ...
    // Extract host with port (for local/dev variants) and host without port (for general checks)
    const hostWithPort = (originalHost || "").toLowerCase().trim();
    const host = hostWithPort.split(":")[0];

    console.log("hostWithPort (lowercased):", hostWithPort);
    ...
    req.originalRequestHost = hostWithPort;
...
function redirectToLanguage(es,en) {
  return catchError(async function(req, res, next) {
    ...
    let hostWithPort = req.originalRequestHost || req.headers['x-forwarded-host'] || req.headers.host || 'localhost';
    hostWithPort = hostWithPort.toLowerCase().trim();

    // For security checks, also derive a host without port
    const hostNoPort = hostWithPort.split(':')[0];
    
    // Log for debugging
    console.log('redirectToLanguage - host:', hostNoPort, 'originalRequestHost:', req.originalRequestHost, 'x-forwarded-host:', req.headers['x-forwarded-host'], 'host header:', req.headers.host);
    
    // Validate host is from sociedad.info domain (security check)
    if (!hostNoPort.includes('sociedad.info') && !hostNoPort.includes('localhost') && helpers.env() !== 'development') {
      console.log('WARNING: Unexpected host detected:', hostNoPort);
    }
    
    // IMPORTANT: keep port when present (especially in development), so redirects
    // like quienesquienwiki.local:3001 preserve the port in the Location header.
    const absoluteURL = `${protocol}://${hostWithPort}${newURL}`;
    console.log('redirectToLanguage - absoluteURL:', absoluteURL);
    
    // Manually set Location header and status to ensure it's not stripped by proxies
    if (!res.headersSent) {
      console.log('Redirecting to: ' + absoluteURL, "from" ,current_url)
      res.status(302);
...
function staticPage(templateName, layout = "layout") {
  return catchError(async function(req, res, next) {
    ...
    if (templateName == "contact") {
      ...
      const redirectUrl = `https://sociedad.info/contact?${params.toString()}`;
