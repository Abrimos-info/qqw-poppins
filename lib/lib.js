...
function entityPage(collection,templateName,idFieldName) {
  return catchError(async function(req, res, next) {
    ...
    // #region agent log
    const adsenseCode = process.env.ADSENSE_NETWORK_CODE;
    try{require('fs').appendFileSync('/home/faras/www/qqw-poppins/.cursor/debug.log',JSON.stringify({location:'lib.js:847',message:'entityPage render - checking home and adsense',data:{isHome:false,hasAdsenseCode:!!adsenseCode,url:req.originalUrl,templateName:templateName},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})+'\n');}catch(e){}
    // #endregion
...
function redirectToSite() {
  return catchError(async function(req, res, next) {
    console.log("redirecToSite",req.headers["x-forwarded-host"]  || req.headers.host);
    const sitename = (req.headers["x-forwarded-host"] || req.headers.host).split(".")[0]
...
function homePage() {
  return catchError(async function(req, res, next) {
    ...
    // #region agent log
    const adsenseCode = process.env.ADSENSE_NETWORK_CODE;
    try{require('fs').appendFileSync('/home/faras/www/qqw-poppins/.cursor/debug.log',JSON.stringify({location:'lib.js:1010',message:'homePage render - checking home and adsense',data:{isHome:true,hasAdsenseCode:!!adsenseCode,url:req.originalUrl},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})+'\n');}catch(e){}
    // #endregion
    res.render('home', {
...
function staticPage(templateName, layout = "layout") {
  return catchError(async function(req, res, next) {
    ...
    // #region agent log
    const adsenseCode = process.env.ADSENSE_NETWORK_CODE;
    try{require('fs').appendFileSync('/home/faras/www/qqw-poppins/.cursor/debug.log',JSON.stringify({location:'lib.js:1067',message:'staticPage render - checking home and adsense',data:{isHome:false,hasAdsenseCode:!!adsenseCode,url:req.originalUrl,templateName:templateName},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})+'\n');}catch(e){}
    // #endregion
   res.render(templateName , {
