 ...
 */
const RATE_LIMITS = {
  // Link-preview crawlers (FB/IG/WhatsApp, LinkedIn, Slack, X, Telegram, Discord, etc.)
  // and AI crawlers we want to be indexed by (GPTBot/SearchGPT). Generous quota since
  // a viral share can produce a brief spike of legitimate fetches; also exempted from
  // country soft-blocks (these often hit from cloud-region IPs like FB-NL).
  preview: {
    maxAttempts: 120,
  ...
  "country:BD": { maxAttempts: 5, windowMs: 5 * 60 * 1000, blockDurationMs: 30 * 60 * 1000 }, // soft — distributed scraping from BD residential proxies (2026-05-20 storm)
  "country:VN": { maxAttempts: 5, windowMs: 5 * 60 * 1000, blockDurationMs: 30 * 60 * 1000 }, // soft — same scrape network as BD, VN exits
  "country:IN": { maxAttempts: 5, windowMs: 5 * 60 * 1000, blockDurationMs: 30 * 60 * 1000 }, // soft — same scrape network, IN exits
};
...
// Link-preview crawlers and AI indexers we want to allow through generously. Matched
// BEFORE BOT_UA_RE so that e.g. GPTBot (which would otherwise match "bot") lands in
// the preview bucket. These UAs are also exempted from country soft-blocks in app.js.
const PREVIEW_UA_RE = /facebookexternalhit|LinkedInBot|Slackbot|Twitterbot|TelegramBot|WhatsApp|Discordbot|SkypeUriPreview|Pinterestbot|GPTBot|OAI-SearchBot|ChatGPT-User|ClaudeBot|Claude-Web|Claude-User|anthropic-ai/i;
// SEO tools, scrapers & any UA containing "bot" — strict limit
const BOT_UA_RE = /bot|semrush|ahref|bytespider|sogou|serpstat|dataforseo|headlesschrome|crawler|hubspot|newsai/i;
// Headless renderers used by search engines
const RENDERER_UA_RE = /Nexus 5X Build\/MMB29P/i;

/**
 * Classify request by User-Agent for rate limiting and analytics.
 * @param {Object} req - HTTP request (with headers)
 * @returns {string|null} "preview" | "crawler" | "bot" | null
 */
function getAgentType(req) {
  ...
  if (PREVIEW_UA_RE.test(ua)) return "preview";
...
function getClientIP(req) {
  // Deflect CDN sets the real client IP here; x-real-ip is the Deflect edge node
  const deflectIP = req.headers["x-deflect-client-ip"];
  if (deflectIP) return deflectIP;

  // Cloudflare sets the real client IP here; x-real-ip is the CF edge node
  const cfIP = req.headers["cf-connecting-ip"];
  if (cfIP) return cfIP;

  // Direct nginx proxy (no higher-level CDN) — x-real-ip is the client
  const realIP = req.headers["x-real-ip"];
  if (realIP) return realIP;

  // X-Forwarded-For fallback — first entry is client-supplied, take the first
...
/** Returns a short 3-char tag for which CDN/proxy path delivered this request. */
function getCDNSource(req) {
  if (req.headers["x-deflect-client-ip"]) return "DFL";
  if (req.headers["cf-connecting-ip"])     return "CF ";
  if (req.headers["x-real-ip"])            return "NGX";
  return "DIR";
}
 ...
 */
function checkRateLimit(key, limitType, ua, country, via) {
  ...
  // Update UA, country, and via on each request (keep the latest)
  if (ua) data.ua = ua;
  ...
  if (data.count > config.maxAttempts) {
    ...
    const viaTag = data.via ? ` | via: ${data.via.trim()}` : "";
    const lines = [
      `⛔ [RATE-LIMIT] Blocked ${storeKey} | Rule: ${config.maxAttempts} reqs/${windowSec}s, blocked for ${blockSec}s${escalationInfo}${countryTag}${viaTag} | ${rateLimitStore.size} tracked IPs`,
    ];
    for (const [k, v] of rateLimitStore.entries()) {
      ...
      if (v.blocked || v.count >= threshold) {
        ...
        const kVia = v.via ? ` ${v.via.trim()}` : "";
        lines.push(`  ${flag}  ${k.padEnd(35)} count: ${String(v.count).padStart(4)}  ttl: ${Math.ceil(((v.blocked ? v.blockUntil : v.resetTime) - now) / 1000)}s  (${rule})${levelTag}${cc}${kVia}  ua: ${uaShort}`);
      }
    }
...
function checkAuthRateLimit(req, limitType, additionalKey = null) {
  ...
  const via = getCDNSource(req);

  // Check IP-based rate limit
  const ipResult = checkRateLimit(ip, limitType, ua, undefined, via);
  if (!ipResult.allowed) {
    return {
      json: {
 ...
 */
function checkSubnetRateLimit(ip, ua, country, via) {
  const subnet24 = getSubnet(ip, 24);
  const result24 = checkRateLimit(subnet24, "subnet24", ua, country, via);
  ...
  const result16 = checkRateLimit(subnet16, "subnet16", ua, country, via);
  if (!result16.allowed) {
